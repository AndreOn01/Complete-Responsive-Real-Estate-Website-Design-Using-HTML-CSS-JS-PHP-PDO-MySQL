...
if(isset($_POST['delete'])){
    ...
    if($verify_request->rowCount() > 0){
        ...
        $warning_msg[] = 'Request delete already!';
    } 
}
...
<section class="requests">
    ...
    <?php
       ...
       if($select_requests->rowCount() > 0){
        ...
        $fetch_propety = $select_properties->fetch(PDO::FETCH_ASSOC);
    ...
    <div class="box">
        ...
        <p>enquery for : <a href="view_property.php?get_id=<?= $fetch_propety['id']; ?>"><?= $fetch_propety['property_name']; ?></a> </p>
        <form action="" method="POST">
            <input type="hidden" name="request_id" value="<?= $fetch_requests['id']; ?>">
            <input type="submit" value="delete request" class="btn" name="delete" onclick="return confirm('delete this request?');">
        </form>
    </div>
    <?php 
        }
